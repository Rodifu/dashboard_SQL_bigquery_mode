...
calc AS (
  SELECT
    ...
    order_value,
    PERCENTILE_CONT(order_value, 0.5)
      OVER (PARTITION BY FORMAT_DATE('%Y-%m', order_date)) AS p50_mes,
    PERCENTILE_CONT(order_value, 0.9)
      OVER (PARTITION BY FORMAT_DATE('%Y-%m', order_date)) AS p90_mes
  FROM order_valueb
)
